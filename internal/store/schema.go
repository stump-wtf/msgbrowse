...
// schemaV21 adds what in-app fact extraction needs to be observable (#366).
//
...
// and a running `msgbrowse serve` sharing one SQLite file, which is what
...
// are separate processes sharing one SQLite file, so this table is their only
// communication channel — without it a run-time Summary evaporates with the
// CLI process that produced it, and a scan that aborted halfway is
// indistinguishable from one that was never started.
//
// It lands in a NEW migration, not an edit to v18: v18 has shipped,
// migrations are append-only (scripts/check-migrations.sh, #217), and v18's
// CREATE TABLE IF NOT EXISTS is inert against a table a live database already
// has. Same rule that moved scan_env to v20 (#385).
//
// ruleset_version, scan_env, address_book and degraded record WHAT ran and
// UNDER WHICH CONDITIONS (REQ-0028-014): scan_env reuses the exact
// "provider/availability" stamp REQ-0028-013 defined in #385 rather than
// deriving a second one, so a run row and the findings it produced can never
// disagree about their environment. degraded is stored even though it is
// derivable from address_book because this row is READ by humans and by the
// web layer without re-encoding that mapping.
//
// The counters are live during the run and final totals after it; updated_at
// is the per-batch heartbeat. finished_at = ” marks a run still in flight;
...
// other runs table.
const schemaV23 = `
CREATE TABLE IF NOT EXISTS spam_runs (
    id                INTEGER PRIMARY KEY,
    started_at        TEXT    NOT NULL,
    updated_at        TEXT    NOT NULL,
    finished_at       TEXT    NOT NULL DEFAULT '',
    ...
    scan_env          TEXT    NOT NULL DEFAULT '',
    address_book      TEXT    NOT NULL DEFAULT '',
    degraded          INTEGER NOT NULL DEFAULT 0,
    conversations     INTEGER NOT NULL DEFAULT 0,
    messages_scanned  INTEGER NOT NULL DEFAULT 0,
    findings          INTEGER NOT NULL DEFAULT 0,
    candidates        INTEGER NOT NULL DEFAULT 0,
    optouts_detected  INTEGER NOT NULL DEFAULT 0,
