...
func (s *Server) scoredFor(ctx context.Context, query string, opts store.SearchOptions) ([]store.ScoredMessage, error) {
	...
	// Every filter the form offers is forwarded, not just the metadata ones:
	// a ticked "Has attachment" must mean the same thing in semantic and
	// hybrid as it does in keyword, or hybrid fuses a filtered keyword half
	// with an unfiltered semantic half and the checkbox quietly stops working.
...
// The body becomes the snippet (no FTS marks on the semantic path); the
// attachment / link glyphs come straight from the vector scan so the SAME
// message renders identically whichever mode (or which half of a hybrid fuse)
// surfaced it, and the score chip is the extra semantic affordance. The body is
// stripped of the FTS highlight
