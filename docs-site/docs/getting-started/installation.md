...
## Install from a branch (testing/development)

You can install the desktop `.app` built from any branch without cutting a
release. The workflow runs on pushes to `main`, tags, and PRs touching the
desktop shell — you can also trigger it manually from the Actions tab.

### From `main` or any branch

1. Go to the [Actions tab](https://github.com/stump-wtf/msgbrowse/actions/workflows/desktop.yml)
   and select the **Desktop** workflow.
2. Click **Run workflow** → choose your branch → **Run workflow**.
3. When the job finishes, download the `msgbrowse-desktop_darwin_universal` artifact.
4. Unzip, then install:
   ```sh
   unzip msgbrowse-desktop_darwin_universal.zip
   cp -r msgbrowse.app /Applications/
   xattr -dr com.apple.quarantine /Applications/msgbrowse.app
   ```
   (The quarantine strip is needed because the CI build is ad-hoc signed, not
   notarized — same as the release.)

### From a PR

The Desktop workflow runs automatically on PRs that change the desktop shell.
The artifact is available under the workflow run on the **Checks** tab of the
PR. Download and install the same way as above.

### Linux

Same workflow — download the `msgbrowse-desktop_linux_amd64` artifact instead.
The binary needs the WebKit2GTK runtime (see the Linux section above).
